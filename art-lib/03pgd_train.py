...
pretrained_model = './model/mnist_um_art.pth'
...
attack = ProjectedGradientDescent(classifier=classifier, eps=0.3)
...
accuracy = np.sum(np.argmax(predictions, axis=1) == np.argmax(y_test, axis=1)) / len(y_test)
print('Accuracy on clean model with pgd test examples: {}%'.format(accuracy * 100))
...
# Step 9: Evaluate the new ART classifier on adversarial test examples

predictions = classifier.predict(x_test_adv)
accuracy = np.sum(np.argmax(predictions, axis=1) == np.argmax(y_test, axis=1)) / len(y_test)
print('Accuracy on new model with pgd test examples: {}%\n'.format(accuracy * 100))

# Step 10: Save trained model

PATH = './model/mnist_pgd_art.pth'
