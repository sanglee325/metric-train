...
accuracy = np.sum(np.argmax(predictions, axis=1) == np.argmax(y_test, axis=1)) / len(y_test)
print('Accuracy on clean model with jsma test examples: {}%'.format(accuracy * 100))
...
predictions = classifier.predict(x_test_adv)
accuracy = np.sum(np.argmax(predictions, axis=1) == np.argmax(y_test, axis=1)) / len(y_test)
print('Accuracy on new model with jsma test examples: {}%'.format(accuracy * 100))

# Step 10: Save trained model

PATH = './model/mnist_jsma_art.pth'
